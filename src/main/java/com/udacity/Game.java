...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        char [] xOrO = {'x','o',};
              ...
              // Wiem, że nie tak to miało wyglądać wiele godzin walczyłem aby zrobić to na pętli zagnieżdzonej, ale niestety nie mogę tego poprawnie napisać cały czas coś nie działą.
            // Jeśli coś jest głupie, ale działa czy nadal jest głupie ? :D
        for (int i = 0; i < xOrO.length; i++) {
            //pionowo
            if (grid[0][0] == xOrO[i] && grid[0][1] == xOrO[i] && grid[0][2] == xOrO[i]) return xOrO[i] + " wins";
            if (grid[1][0] == xOrO[i] && grid[1][1] == xOrO[i] && grid[1][2] == xOrO[i]) return xOrO[i] + " wins";
            if (grid[2][0] == xOrO[i] && grid[2][1] == xOrO[i] && grid[2][2] == xOrO[i]) return xOrO[i] + " wins";
        }
        for (int i = 0; i < xOrO.length; i++) {
            //poziomo
            if (grid[0][0] == xOrO[i] && grid[1][0] == xOrO[i] && grid[2][0] == xOrO[i]) return xOrO[i] + " wins";
            if (grid[0][1] == xOrO[i] && grid[1][1] == xOrO[i] && grid[2][1] == xOrO[i]) return xOrO[i] + " wins";
            if (grid[0][2] == xOrO[i] && grid[1][2] == xOrO[i] && grid[2][2] == xOrO[i]) return xOrO[i] + " wins";
        }
            //przekątne
        for (int i = 0; i < xOrO.length; i++) {
            if (grid[0][0] == xOrO[i] && grid[1][1] == xOrO[i] && grid[2][2] == xOrO[i]) return xOrO[i] + " wins";
            if (grid[0][2] == xOrO[i] && grid[1][1] == xOrO[i] && grid[2][0] == xOrO[i]) return xOrO[i] + " wins";
        }
                //tragiczny remis
